...
Mount_WebUI(){
	...
	echo "vnstat" > "$SCRIPT_WEBPAGE_DIR/$(echo $MyPage | cut -f1 -d'.').title"
	...
	if [ "$(uname -o)" = "ASUSWRT-Merlin" ]; then
		...
		sed -i "/url: \"javascript:window.open('\/ext\/shared-jy\/redirect.htm'/i {url: \"$MyPage\", tabName: \"vnstat\"}," /tmp/menuTree.js
